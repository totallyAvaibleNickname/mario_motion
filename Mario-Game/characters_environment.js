//v SEÇÃO CHECADA E MARCADA COMO CONCLUÍDA v//
/*=================================
=            Variáveis            =
=================================*/
...
GameStatus = "";
function game(){
  console.log("Posição X do nariz:" + noseX + "Posição Y do nariz:" + noseY);
  instializeInDraw();
...
function changeGameStatud(character){
  if(noseX =! "" &&  gameConfig.status === "start" && GameStatus == "start") {
    document.getElementById("statua").innerHTML = "Carregado.";
    world_start.play();
...
function die(character){
    ...
    console.log("Mario morreu :(:(:(:(" + character.liveNumber);
